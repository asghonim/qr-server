...
CREATE OR REPLACE FUNCTION private.on_insert_contact_submissions()  RETURNS TRIGGER AS $$ BEGIN NEW.created_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION private.on_insert_contact_messages()     RETURNS TRIGGER AS $$ BEGIN NEW.created_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION private.on_insert_contact_attachments()  RETURNS TRIGGER AS $$ BEGIN NEW.created_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION private.on_insert_outbox_events()        RETURNS TRIGGER AS $$ BEGIN NEW.created_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
...
CREATE INDEX idx_contact_assigned        ON contact_submissions(assigned_to_user_id);
CREATE INDEX idx_contact_authed_user     ON contact_submissions(authenticated_user_id);
    ...
    USING (
        ...
        AND EXISTS (
            ...
            WHERE cs.id = submission_id
              AND cs.authenticated_user_id = auth.uid()
        )
    );
